...
;;; -------------------------------------------------------------------------------------
;;;    7/17/87  PMH       :get-item and :insert-item were not calculating position correctly
...
;;;			Put in some DECLARE SPECIALs inside of SCROLL-REDISPLAY-DISPLAY-ITEM so that those
;;;			variables would not generate a compiler warning.
;;; 10/03/86   GRH	Scroll-bar-mixin -> w:Scroll-bar-mixin
;;; 10/01/86   GRH	Use scroll-bar-mixin instead of obsolete basic-scroll-bar in scroll-window flavor.
;;; 09/10/86   DLS	Changed the scroll window redisplay methods and functions to use the window ERASE-ALUF
;;;			to erase rectangles instead of a hard coded ALU-ANDCA to fix problems with reverse video
;;;			scroll windows.
...
;;; 04/08/86   TWE	Changed SCROLL-MAKE-ENTRY to use LOOP instead of fillarray.

;;; A scroll window displays a section of a database.  The user can
;;; dynamically change what is displayed on the window.

;;; The datastructure consists of items.  Each item represents an
;;; integral number of lines.

;;; An item is either 1) a list of items or 2) an array which contains
;;; entries.  You can create an array-item using the function
;;; SCROLL-PARSE-ITEM.  The argument is a list which contains options
;;; followed by descriptions of entries.  The options are :MOUSE
;;; <mouse-data>, :MOUSE-SELF <mouse-data> and :LEADER <leader-data>.
;;; :MOUSE specifies the SCROLL-ITEM-LINE-SENSITIVITY.  :MOUSE-SELF is
;;; similar except that the symbol SELF is replaced where it occurs by the
;;; item array.  :LEADER is either a number of extra leader elements to
;;; construct or a list of contents for extra leader elements to construct.

;;; The array elements of an array item are filled with objecs of type
;;; SCROLL-ENTRY.  When you specify an item using SCROLL-PARSE-ITEM you
;;; follow the options with a list of entry-descriptors which are parsed
;;; with SCROLL-INTERPRET-ENTRY to get the entry-arrays which are stored in
;;; the item array.
...
;;; 	   The car of the list can be the symbol :MOUSE, followed by the
;;; 	   mouse-data-list or :MOUSE-ITEM, which is like :MOUSE except
;;; 	   that the symbol ITEM is replaced by the current item
;;; 	   throughout the mouse-data-list.  After any of these, the
;;; 	   entry must start with :STRING, :SYMEVAL, :VALUE or :FUNCTION
;;; 	:STRING <string> <width-of-item-in-pixels>
;;; 	:SYMEVAL <symbol> <width-of-item-or-NIL-if-varies>
;;; 	   (<format-string for printing value>).
;;; 	:VALUE <number> <width-of-item or NIL if varies> (<format string
;;; 	   for printing value>).  The value is the <number>'th element
;;; 	   of the window's :VALUE-ARRAY.
;;; 	:FUNCTION <function> <args for that function> <width-of-item or
;;; 	   nil if varies> (<format-string for printing value>).  The
;;; 	   function is passed the appropriate args as you specified them
...
(DEFSTRUCT (SCROLL-ITEM-LEADER (:CONSTRUCTOR NIL) (:SIZE-SYMBOL SCROLL-ITEM-LEADER-OFFSET)
  (:CONC-NAME NIL) (:CALLABLE-CONSTRUCTORS NIL) (:ALTERANT ALTER-SCROLL-ITEM-LEADER)
  (:PREDICATE NIL) (:COPIER NIL) (:TYPE :ARRAY-LEADER))
  SCROLL-ITEM-MOUSE-ITEMS
  SCROLL-ITEM-LINE-SENSITIVITY) 

;;; Accessors of a list item.
(DEFSTRUCT (SCROLL-ITEM (:CONSTRUCTOR NIL) (:CONC-NAME NIL) (:CALLABLE-CONSTRUCTORS NIL)
  (:ALTERANT ALTER-SCROLL-ITEM) (:PREDICATE NIL) (:COPIER NIL) (:TYPE :LIST*))
  SCROLL-ITEM-PLIST
...
(DEFSTRUCT (SCROLL-ENTRY (:CONC-NAME NIL) (:CALLABLE-CONSTRUCTORS NIL) (:ALTERANT ALTER-SCROLL-ENTRY)
...
(DEFMETHOD (BASIC-SCROLL-WINDOW :BEFORE :INIT) (PLIST)
  (SETQ TOP-ITEM NIL
	TARGET-TOP-ITEM 0)
  (OR VALUE-ARRAY
      (SETQ VALUE-ARRAY (MAKE-ARRAY SCROLL-DEFAULT-VALUE-ARRAY-SIZE)))
  (SETF (GET PLIST :BLINKER-P) NIL)) 
...
(DEFUN SCROLL-REDISPLAY-DISPLAY-ITEM
  ...
  "Display item ITEM with item number CURRENT-COUNT, if it belongs
	  ...
	  (SETQ OLD-LINE
		(DO ((I FIRST-LINE (1+ I)))
		  ...
		  (AND (EQ ITEM (AREF SCREEN-IMAGE I 0))
		       ...
		       ;; If first line of this item on screen anywhere,
		       ;; then can move it to current line.
		       (RETURN I))))
	...
	(SEND SELF :DELETE-LINE (- OLD-LINE FIRST-LINE))))
	 ...
	 (PROGN
	   (DOTIMES (I (SCROLL-ITEM-SIZE ITEM))
			 ...
			 (= (SCROLL-ENTRY-WIDTH ENTRY)
			    (SETQ WID
				  (FUNCALL (SCROLL-ENTRY-FUNCTION ENTRY) :WIDTH ENTRY
					   (SCROLL-ENTRY-FUNCTION ENTRY))))))
		  ;; Going to kill line, move it down if it
		  ;; belongs below here anyway.
		 ...
		 (T
		  ;; Fixed width entry, or variable width entry
		  ;; and width hasn't changed.  Using the width,
		  ;; figure out the cursor motion and erase area.
		  (MULTIPLE-VALUE-BIND (FINAL-X FINAL-Y FINAL-COUNT)
			...
			(COND
			  ((OR
			     ...
			     (NULL LE))
			   ;; We know about this line so just clear
			   ;; the area.
			   (%DRAW-RECTANGLE
			     (- (IF (= Y FINAL-Y)
				    FINAL-X
			  ...
			  ((EQ LE ITEM)
			   ;; We own line, but it is wrong number.
			   ;; Clear the line and flush all knowledge.
			   (%DRAW-RECTANGLE (- (SHEET-INSIDE-RIGHT) X) LINE-HEIGHT X
					    Y erase-aluf SELF)
			   (SCROLL-FLUSH-ITEM-FROM-SCREEN-IMAGE ITEM))
			  (T
			   ;; Make room for remaining number of
			   ;; lines and return.
			   (SETQ DELTA-ITEMS
				 (- (AREF SCREEN-IMAGE (+ CURRENT-LINE LINE) 2)
				    CURRENT-COUNT))
			   ;; DELTA-ITEMS is a guess as to the
			   ;; number of items in between this
			   ;; and the line it collided with.
			   ;; Assuming one line per item, this
			   ;; is a good guess as to the number
			   ;; of additional lines to insert.
			   (LET-GLOBALLY ((CURSOR-Y Y))
			     (SEND SELF
				   :INSERT-LINE
				   ;; If we are past the item
				   ;; that's on this line, it
				   ;; can't possibly appear on the
				   ;; screen -- insert enough
				   ;; lines to make it go off the
				   ;; screen.
				   (MAX 1
					(MIN
...
(DEFUN SCROLL-REDISPLAY-ITEM-LOOP (ITEM CURRENT-COUNT FUNCTION NO-RECOMP
				   &REST POSITION &AUX FUN)
  "Loop over an item and its inferiors until TARGET-TOP-ITEM has
been reached, then start doing the appropriate things to fix up the
screen.  This may require inserting and deleting lines, etc...  Returns
what the number of the next item is.

NO-RECOMP says do not recompute the inferiors of ITEM; redisplay
		whatever inferiors are recorded for it.
FUNCTION should be the function to redisplay one item, such as
		SCROLL-REDISPLAY-DISPLAY-ITEM."
  (DECLARE (:SELF-FLAVOR BASIC-SCROLL-WINDOW))
  (COND ((NULL ITEM))
	((CONSP ITEM)
	 ;; A list of other items, recurse
	 (OR NO-RECOMP
	     (DO ((F (SCROLL-ITEM-PLIST ITEM) (CDDR F)))
		 ((NULL F))
	       (CASE (CAR F)
		     (:FUNCTION (SETQ FUN (CADR F)))
		     (:PRE-PROCESS-FUNCTION (FUNCALL (CADR F) ITEM)))))
	 (DO ((ITEMS (SCROLL-ITEM-COMPONENT-ITEMS ITEM) (CDR ITEMS)))
	     ((NULL ITEMS))
	   (AND FUN
		(RPLACA ITEMS (FUNCALL FUN (CAR ITEMS) POSITION
				       (LOCF (SCROLL-ITEM-PLIST ITEM)))))
	   (SETQ CURRENT-COUNT
		 (APPLY #'SCROLL-REDISPLAY-ITEM-LOOP (CAR ITEMS) CURRENT-COUNT FUNCTION NO-RECOMP
			0 POSITION))
	   (SEND SELF :HANDLE-EXCEPTIONS)
...
(DEFUN SCROLL-REDISPLAY (&OPTIONAL FULL-REDISPLAY
			 &AUX (SCROLL-NEW-X 0) (SCROLL-NEW-Y 0))
  "Redisplay SELF, a scroll window.  FULL-REDISPLAY says redisplay
all, else only changes."
  (DECLARE (:SELF-FLAVOR BASIC-SCROLL-WINDOW))
  (SHEET-FORCE-ACCESS (SELF :NO-PREPARE)
     (LOCK-SHEET (SELF)
       (AND (OR FULL-REDISPLAY
		(NULL TOP-ITEM))
	...
	(CATCH 'END-OF-PAGE
	  (PROGN
	    (SETQ BOTTOM-ITEM -1)
	    (SCROLL-REDISPLAY-ITEM-LOOP DISPLAY-ITEM 0 #'SCROLL-REDISPLAY-DISPLAY-ITEM NIL 0)
	    (SETQ BOTTOM-ITEM (1+ BOTTOM-ITEM))
	    (AND SCROLL-NEW-X (SEND SELF :SET-CURSORPOS SCROLL-NEW-X SCROLL-NEW-Y))
	    (SEND SELF :CLEAR-EOL)
	    (DO ((I (SCROLL-LINE) (1+ I)))
		((>= I SCREEN-LINES))
		 ;; This does not :TYO a #\CR because SHEET-CR-NOT-NEWLINE-FLAG
		 ;; may be set, and we really want to move to the next line.
	      (CATCH 'END-OF-PAGE
		(SHEET-CRLF SELF))
	      (SEND SELF :CLEAR-EOL)
	      (SETF (AREF SCREEN-IMAGE I 0) NIL)
...
(DEFUN SCROLL-ENTRY-CONSTANT-STRING-FUNCTION (OP ENTRY &OPTIONAL IGNORE)
  (DECLARE (:SELF-FLAVOR BASIC-SCROLL-WINDOW))
  (CASE OP
     (:PRINT 
...
(DEFUN SCROLL-ENTRY-SYMBOL-VALUE-FUNCTION (OP ENTRY &OPTIONAL US)
  (DECLARE (:SELF-FLAVOR BASIC-SCROLL-WINDOW))
  (CASE OP
     (:PRINT
      (LET ((DATA (SCROLL-ENTRY-PRINTED-FORM ENTRY))
	    (FORMAT (SCROLL-ENTRY-PRINT-FORMAT ENTRY)))
	(COND
	  (DATA (SEND SELF :STRING-OUT DATA))
	  (T
	   (LET ((*PRINT-BASE* (OR (CADR FORMAT) *PRINT-BASE*))
		 (*NOPOINT (OR (CADDR FORMAT) *NOPOINT)))
	     (SETQ DATA (SCROLL-ENTRY-DATA ENTRY))
	     (IF (CAR FORMAT)
		 ;; Convert this to Common-lisp format when arg is
		 ;; Common-lisp compatible control string,
		 ;; else this makes Peek Areas mode display wrong.
		 (FORMAT SELF (CAR FORMAT) DATA)
		 (FORMAT SELF "~A" DATA)))))))
     (:CHANGED-P 
      (COND
	((NOT (EQUAL (SCROLL-ENTRY-DATA ENTRY) (FUNCALL US :RECOMPUTE ENTRY)))
	 (SETF (SCROLL-ENTRY-PRINTED-FORM ENTRY) NIL) T)
	(T NIL)))
     (:RECOMPUTE
      (LET (DATA)
	(SETQ DATA (SYMBOL-VALUE (SCROLL-ENTRY-RECOMPUTE-FUNCTION ENTRY)))
	(SETF (SCROLL-ENTRY-DATA ENTRY) DATA) DATA))
     (:WIDTH
      (LET (DATA
	    (FORMAT (SCROLL-ENTRY-PRINT-FORMAT ENTRY)))
...
(DEFUN SCROLL-ENTRY-CALL-FUNCTION-FUNCTION (OP ENTRY &OPTIONAL US)
  (DECLARE (:SELF-FLAVOR BASIC-SCROLL-WINDOW))
  (CASE OP
     (:RECOMPUTE
      (LET ((DATA (APPLY (SCROLL-ENTRY-RECOMPUTE-FUNCTION ENTRY) (SCROLL-ENTRY-ARGS ENTRY))))
	(SETF (SCROLL-ENTRY-DATA ENTRY) DATA) DATA))
     (OTHERWISE
      (SCROLL-ENTRY-SYMBOL-VALUE-FUNCTION OP ENTRY US))))

...
(DEFUN SCROLL-TOTAL-ITEMS ()
  ...
  (DECLARE (:SELF-FLAVOR BASIC-SCROLL-WINDOW))
  (1- (SCROLL-REDISPLAY-ITEM-LOOP DISPLAY-ITEM 0 #'(LAMBDA (&REST IGNORE))
				  T 0))) 

(DEFMETHOD (BASIC-SCROLL-WINDOW :SCROLL-POSITION) ()
  (BLOCK () (RETURN (OR TOP-ITEM 0)		;Item number at top
...
(DEFMETHOD (BASIC-SCROLL-WINDOW :REDISPLAY)
...
(DEFMETHOD (BASIC-SCROLL-WINDOW :REDISPLAY-SELECTED-ITEMS) (ITEMS)
  (SHEET-FORCE-ACCESS (SELF :NO-PREPARE)
	...
	(DOTIMES (I SCREEN-LINES)
	  (COND
	    ((AND (SETQ ITEM (CAR (MEMBER (AREF SCREEN-IMAGE I 0) ITEMS :TEST #'EQ)))
		  (ZEROP (AREF SCREEN-IMAGE I 1)))
...
(DEFMETHOD (BASIC-SCROLL-WINDOW :AFTER :CHANGE-OF-SIZE-OR-MARGINS)
           (&REST IGNORE)
  (SCROLL-MAKE-SCREEN-IMAGE))

(DEFMETHOD (BASIC-SCROLL-WINDOW :AFTER :CHANGE-OF-DEFAULT-FONT)
           (&REST IGNORE)
  (SCROLL-MAKE-SCREEN-IMAGE)
  (SHEET-FORCE-ACCESS (SELF :NO-PREPARE)
    (SEND SELF :CLEAR-SCREEN)))
...
(DEFMETHOD (BASIC-SCROLL-WINDOW :BEFORE :END-OF-LINE-EXCEPTION) ()
  ...
  (COND
    (TRUNCATION (THROW 'END-OF-LINE T)))
  (COND
    ...
    ((NOT (= (1+ *SCROLL-CURRENT-ITEM-LINE*)
	     (AREF SCREEN-IMAGE (1+ (SCROLL-LINE)) 1)))
     (SCROLL-FLUSH-ITEM-FROM-SCREEN-IMAGE *SCROLL-CURRENT-ITEM*)))) 

(DEFMETHOD (BASIC-SCROLL-WINDOW :BEFORE :DELETE-LINE)
	   (&OPTIONAL (N 1) &AUX (CUR-LINE (SCROLL-LINE)))
  ;; Fill in the hole.
  (DO ((L CUR-LINE (1+ L)))
      ((>= (+ L N) SCREEN-LINES))
...
(DEFMETHOD (BASIC-SCROLL-WINDOW :BEFORE :INSERT-LINE) (&OPTIONAL (N 1) &AUX (CUR-LINE (SCROLL-LINE)))
...
(DEFUN SCROLL-ITEM-NUMBER-AT-Y (Y &AUX IN)
  "Return the item number of the item displayed at Y-position Y, as of
last redisplay."
  (DECLARE (:SELF-FLAVOR BASIC-SCROLL-WINDOW))
  ;; Convert the pixel number to a line number.
...
(DEFUN SCROLL-FIND-A-TOP-ITEM (TARGET-ITEM TARGET-LINE
  ...
  "Find which item should be the top item to put TARGET-ITEM on
screen line TARGET-LINE."
  (DECLARE (:SELF-FLAVOR BASIC-SCROLL-WINDOW)
...
(DEFUN SCROLL-FIND-A-TOP-ITEM-INTERNAL (ITEM ITEM-NUMBER)
  (DECLARE (:SELF-FLAVOR BASIC-SCROLL-WINDOW)
     (SPECIAL SCROLL-ITEM-LIST SCROLL-ITEM-LONGEST-DISTANCE SCROLL-ITEM-TARGET-DISTANCE
	SCROLL-ITEM-TARGET-ITEM))
  (COND
    ((= ITEM-NUMBER SCROLL-ITEM-TARGET-ITEM)
    ...
    ((>= SCROLL-ITEM-LONGEST-DISTANCE SCROLL-ITEM-TARGET-DISTANCE)
     ;; We have enough items to make up the distance, throw away
     ;; the last one and add in the new one.
     (LET ((LAST-ITEM (CAR (LAST SCROLL-ITEM-LIST))))
       ...
       (SETQ SCROLL-ITEM-LIST (DELETE LAST-ITEM (THE LIST SCROLL-ITEM-LIST) :TEST #'EQ))
...
(DEFMETHOD (BASIC-SCROLL-WINDOW :SCROLL-RELATIVE) (FROM TO)
  ...
  (SETQ TO (IF (EQ TO :TOP)
	       FROM
             ;; Find an item such that if we put it on the top, then the
             ;; FROM item will be in the desired position.  This is an
             ;; estimate only and not guaranteed to do exactly the right
             ;; thing.
...
(DEFMETHOD (SCROLL-WINDOW-WITH-TYPEOUT-MIXIN :BEFORE :REDISPLAY) (&REST IGNORE)
  "If the typeout window is active, deexposed it, and make sure the
redisplayer knows how many lines were clobbered."
  (COND
    ((SEND TYPEOUT-WINDOW :ACTIVE-P)
     (LET ((BR
	    (MIN SCREEN-LINES (1+ (TRUNCATE (SEND TYPEOUT-WINDOW :BOTTOM-REACHED)
					    LINE-HEIGHT)))))
       (DOTIMES (L BR)
	;; Mark lines as clobbered
	 ...
	 (SETF (AREF SCREEN-IMAGE L 2) -1))
       ...
       (SEND SELF :DRAW-RECTANGLE (SHEET-INSIDE-WIDTH) (* BR LINE-HEIGHT) 0 0 erase-aluf))))) 
...
(DEFUN SCROLL-GET-ITEM-LOCATIVE (POSITION
				 &AUX (ITEM (LOCATE-IN-INSTANCE SELF 'DISPLAY-ITEM)))
  (DECLARE (:SELF-FLAVOR BASIC-SCROLL-WINDOW))
  (AND (NUMBERP POSITION)
       (SETQ POSITION (CONS POSITION NIL)))
  (DOLIST (C POSITION)
    (SETQ ITEM (LOCF (NTH C (SCROLL-ITEM-COMPONENT-ITEMS (CONTENTS ITEM))))))
  ITEM)
...
(DEFMETHOD (BASIC-SCROLL-WINDOW :INSERT-ITEM) (POSITION ITEM &AUX WHERE ITEM-EXISTS)
  "Inserts an item before the specified position."
  (AND (NUMBERP POSITION)
       (SETQ POSITION (CONS POSITION NIL)))
  (SETQ WHERE (LOCATE-IN-INSTANCE SELF 'DISPLAY-ITEM))
  (DOLIST (c POSITION)
    (setf item-exists (nthcdr c (SCROLL-ITEM-COMPONENT-ITEMS (contents where))))
    (if (consp item-exists)
	(setf where (locf (car item-exists)))
	(ferror "Position ~a does not exist." position)))
  (rplacd ITEM-EXISTS (CONS (car ITEM-EXISTS) (cdr ITEM-EXISTS)))
  (rplaca ITEM-EXISTS item)
  (SEND SELF :REDISPLAY))
...
(DEFUN SCROLL-MAKE-ENTRY (&REST ELTS &AUX ENTRY)
  ...
  ;; We go through this contortion to implement the FILLARRAY function.
  ;; What we are doing is filling the ENTRY to have an item from ELTS in
  ;; each element.
  (LOOP FOR INDEX FROM 0 BELOW (LENGTH ENTRY)
	FOR ELEMENT IN ELTS
	DO (SETF (AREF ENTRY INDEX) ELEMENT)
	FINALLY (WHEN (< INDEX (LENGTH ENTRY))
		  ;; There weren't enough items in ELTS.  Extend ENTRY
		  ;; out to contain the last item in ELTS for the
		  ;; remaining elements.
...
(DEFUN SCROLL-INTERPRET-ENTRY (DESCRIPTOR ITEM &AUX MOUSE)
  "Given a descriptor (see documentation) returns an entry made by
parsing it.  The entry is suitable for inclusion in an array-type item.
ITEM is the item you are planning to use the entry in."
  (COND-EVERY
    ((STRINGP DESCRIPTOR)
     (SETQ DESCRIPTOR (LIST :STRING DESCRIPTOR)))
    ((SYMBOLP DESCRIPTOR)
     (SETQ DESCRIPTOR (LIST :SYMEVAL DESCRIPTOR)))
    ((OR (= (%DATA-TYPE DESCRIPTOR) DTP-FEF-POINTER)
	 (AND (CONSP DESCRIPTOR)
	      (MEMBER (CAR DESCRIPTOR) SI::FUNCTION-START-SYMBOLS :TEST #'EQ)))
     (SETQ DESCRIPTOR (LIST :FUNCTION DESCRIPTOR)))
    ((CONSP DESCRIPTOR)
     (COND-EVERY
       ((EQ (FIRST DESCRIPTOR) :MOUSE)
	...
	(SETQ DESCRIPTOR `(:FUNCTION ,#'VALUE (,(SECOND DESCRIPTOR)) 
				     ,@(CDDR DESCRIPTOR)))))
     (SETQ DESCRIPTOR
	   (CASE (FIRST DESCRIPTOR)
		 (:STRING
		  (SCROLL-MAKE-ENTRY 'SCROLL-ENTRY-CONSTANT-STRING-FUNCTION NIL NIL 0 0 0
				     (OR (THIRD DESCRIPTOR)
					 (LENGTH (SECOND DESCRIPTOR))) NIL
				     (SECOND DESCRIPTOR) NIL NIL NIL))
		 (:SYMEVAL
		  (SCROLL-MAKE-ENTRY 'SCROLL-ENTRY-SYMBOL-VALUE-FUNCTION (SECOND DESCRIPTOR) NIL 0 0 0
				     (OR (THIRD DESCRIPTOR) 0)
				     (NULL (THIRD DESCRIPTOR))
				     (CONS NIL NIL)  NIL
				     (FOURTH DESCRIPTOR) NIL))
		 (:FUNCTION
		  (SCROLL-MAKE-ENTRY 'SCROLL-ENTRY-CALL-FUNCTION-FUNCTION (SECOND DESCRIPTOR)
				     (THIRD DESCRIPTOR) 0 0 0
				     (OR (FOURTH DESCRIPTOR) 0)
				     (NULL (FOURTH DESCRIPTOR))
				     (CONS NIL NIL) NIL (FIFTH DESCRIPTOR)
				     NIL)))))
    (MOUSE (SETF (SCROLL-ENTRY-MOUSE-INFO DESCRIPTOR) MOUSE))
    (OTHERWISE (FERROR NIL "Unknown kind of descriptor: ~S" DESCRIPTOR)))
  DESCRIPTOR)

(DEFUN SCROLL-PARSE-ITEM (&REST ITEM-SPEC
			  &AUX ITEM (EXTRA-LEADER 0) LEADER-FILL MOUSE MOUSE-SELF)
  "Given a list of entry descriptors, produce an array-type item."
  (SETQ ITEM-SPEC (DELETE NIL (THE LIST (COPY-LIST ITEM-SPEC)) :TEST #'EQ))
  (DO ((L ITEM-SPEC (CDDR L)))
      ((NOT (SYMBOLP (CAR L)))
       (SETQ ITEM-SPEC L))
    (CASE (CAR L)
      (:MOUSE (SETQ MOUSE (CADR L)))
      (:MOUSE-SELF (SETQ MOUSE-SELF T
			 MOUSE (CADR L)))
      (:LEADER
       (SETQ EXTRA-LEADER
	     (IF (NUMBERP (CADR L))
  ...
  (SETQ ITEM
	(MAKE-ARRAY (LENGTH ITEM-SPEC) :LEADER-LENGTH (+ EXTRA-LEADER SCROLL-ITEM-LEADER-OFFSET)))
  (AND MOUSE-SELF
       (SETQ MOUSE (SUBLIS `((SELF . ,ITEM)) MOUSE)))
  (AND LEADER-FILL
...
(DEFUN SCROLL-STRING-ITEM-WITH-EMBEDDED-NEWLINES (STRING &AUX STRINGS ITEM)
  (DO ((NEXT (POSITION #\NEWLINE (THE STRING (STRING STRING)) :TEST #'CHAR-EQUAL)
	(POSITION #\NEWLINE (THE STRING (STRING STRING)) :START (1+ NEXT) :TEST #'CHAR-EQUAL))
       (ONEXT 0 (1+ NEXT)))
      ((NULL NEXT)
       (AND (< ONEXT (LENGTH STRING))
...
(DEFUN SCROLL-MAINTAIN-LIST-UNORDERED
       (INIT-FUN ITEM-FUN &OPTIONAL PER-ELT-FUN STEPPER)
  "Given a function that returns a list, and a function that returns an
item spec when given an element of that list, maintains one item for
each element in the list.  This is not useful when recursion is
necessary.  Returns an item that should be inserted somewhere.  The
LIST-FUN should return a private copy of the list."
...
(DEFUN SCROLL-MAINTAIN-LIST-STEPPER (STATE)
  (DECLARE)
  (block nil
    (RETURN (CAR STATE) (CDR STATE) (NULL (CDR STATE))))) 

(DEFSTRUCT (STATE (:CONSTRUCTOR NIL) (:CONC-NAME NIL) (:CALLABLE-CONSTRUCTORS NIL) (:ALTERANT ALTER-STATE)
  (:PREDICATE NIL) (:COPIER NIL) (:TYPE :LIST))
  STATE-VALUE
  STATE-FLAG
  STATE-ITEM) 

(DEFUN SCROLL-MAINTAIN-LIST-UNORDERED-UPDATE-FUNCTION
       (ITEM &AUX (FLAGS-PLIST (LOCF (CAR ITEM))))
  (LET ((STEP-STATE (FUNCALL (GET FLAGS-PLIST :INIT-FUNCTION)))
    ...
    ;; Loop over all items.  If one is found that doesn't exist, add it
    ;; and remember that.  Any that no longer exist need to be flushed.
    (DO ((CURRENT)
	 (LAST)
	 (STATE))
	((OR (NULL STEP-STATE) LAST))
      (MULTIPLE-VALUE-SETQ (CURRENT STEP-STATE LAST)
	(FUNCALL STEPPER STEP-STATE))
      (IF (SETQ STATE (ASSOC CURRENT OLD-STATE :TEST #'EQ))
	  ...
	  (LET ((NEW-ITEM (FUNCALL ITEM-FUN CURRENT)))
	    ...
	    (PUSH NEW-ITEM (SCROLL-ITEM-COMPONENT-ITEMS ITEM)))))
    (DOLIST (STATE OLD-STATE)
     ;; Delete all items that are no longer valid
      (COND
	((NOT (STATE-FLAG STATE))
	 (SETF (SCROLL-ITEM-COMPONENT-ITEMS ITEM)
	       (DELETE (STATE-ITEM STATE) (THE LIST (SCROLL-ITEM-COMPONENT-ITEMS ITEM)) :TEST #'EQ))
	 (SETQ OLD-STATE (DELETE STATE (THE LIST OLD-STATE) :TEST #'EQ)))))
    ;; ITEM and OLD-STATE have been updated.  Store back appropriate info.
    (SETF (GET FLAGS-PLIST :OLD-STATE) OLD-STATE)
    ITEM)) 

(DEFUN SCROLL-MAINTAIN-LIST
       (INIT-FUN ITEM-FUN
        &OPTIONAL
        PER-ELT-FUN
        STEPPER
...
(DEFVAR SCROLL-LIST-AREA working-storage-area
  ;don't use customized area PMH 2/25/88 this was (MAKE-AREA :NAME 'SCROLL-LIST-AREA :REPRESENTATION :LIST))
  "This area contains scroll item data.")

(DEFUN SCROLL-MAINTAIN-LIST-UPDATE-FUNCTION
       (ITEM &AUX (FLAGS-PLIST (LOCF (CAR ITEM))))
  (LET* ((STEP-STATE (FUNCALL (GET FLAGS-PLIST :INIT-FUNCTION)))
	 ...
	 (ITEMS (LOCF (SCROLL-ITEM-COMPONENT-ITEMS ITEM))))
	...
	(LAST
	 ...
	 (COND
	   ((AND COMPACT-P NEEDS-COMPACTION)
	    (SETF (SCROLL-ITEM-COMPONENT-ITEMS ITEM) (COPY-LIST (SCROLL-ITEM-COMPONENT-ITEMS ITEM) SCROLL-LIST-AREA))
	    (SETF (GET FLAGS-PLIST :OLD-STATE)
		  (COPY-LIST (GET FLAGS-PLIST :OLD-STATE) SCROLL-LIST-AREA)))))
	...
	((MEMBER CURRENT OLD-STATE :TEST #'EQ)
	...
	(T (SETQ NEEDS-COMPACTION T)
	 (RPLACD PREV-STATE
		 (SETQ OLD-STATE (CONS-IN-AREA CURRENT (CDR PREV-STATE) SCROLL-LIST-AREA)))
	 (RPLACD PREV-ITEM
		 (SETQ ITEMS
		       (CONS-IN-AREA (FUNCALL ITEM-FUN CURRENT) (CDR PREV-ITEM) SCROLL-LIST-AREA)))))
      (AND OLD-STATE
	   (SETQ PREV-STATE OLD-STATE))
      (AND ITEMS
	   (SETQ PREV-ITEM ITEMS)))
    ITEM)) 

(DEFUN SCROLL-MAINTAIN-LIST-UPDATE-STATES
       (STATES WINDOW
	&OPTIONAL (DITEM (SEND WINDOW :DISPLAY-ITEM)) &AUX
  (FLAGS-PLIST (LOCF (CAR DITEM))))
  (LET ((OLD-STATE (LOCF (GET FLAGS-PLIST :OLD-STATE)))
	(ITEMS (LOCF (SCROLL-ITEM-COMPONENT-ITEMS DITEM)))
	(ITEMS-TO-BE-REDISPLAYED NIL))
    (DOLIST (STATE OLD-STATE)
      (AND (MEMBER STATE STATES :TEST #'EQ)
...
(DEFFLAVOR ESSENTIAL-SCROLL-MOUSE-MIXIN
  ...
  (:DOCUMENTATION
    :MIXIN
    "Makes a scroll window suport mouse-sensitive items and entries."))
...
(DEFMETHOD (ESSENTIAL-SCROLL-MOUSE-MIXIN :AFTER :INIT) (IGNORE)
  (SETQ ITEM-BLINKER
	(APPLY #'MAKE-BLINKER SELF (OR (CAR ITEM-BLINKER) 'HOLLOW-RECTANGULAR-BLINKER)
	       :VISIBILITY () (CDR ITEM-BLINKER)))) 

(DEFMETHOD (ESSENTIAL-SCROLL-MOUSE-MIXIN :AFTER :HANDLE-MOUSE) ()
  (BLINKER-SET-VISIBILITY ITEM-BLINKER NIL)
  (SETQ CURRENT-ITEM NIL))

(DEFSTRUCT (SCROLL-MOUSE-ITEM (:CONSTRUCTOR NIL) (:CONC-NAME NIL) (:CALLABLE-CONSTRUCTORS NIL)
  (:ALTERANT ALTER-SCROLL-MOUSE-ITEM) (:PREDICATE NIL) (:COPIER NIL) (:TYPE :LIST))
...
(DEFUN SCROLL-FIND-SENSITIVE-ITEM (X Y)
  "Return info on the MOUSE-SENSITIVE ITEM surrounding position X,
Y in a scroll window.  A MOUSE-SENSITIVE ITEM is not the same
thing as an ITEM in the sense of a scroll-window; it is anything that
makes an area mouse sensitive.  The first value is the
 (SCROLL-ENTRY-MOUSE-INFO ...) of an entry, or the
SCROLL-MOUSE-ITEM-ITEM of a line, or the
SCROLL-ITEM-LINE-SENSITIVITY of a scroll window item.  The second
value is a keyword, the mouse-sensitive item type.  The next four
values are the X, Y, WIDTH and HEIGHT of the sensitive area."
  (DECLARE (VALUES SENSITIVE-ITEM SENSITIVE-ITEM-TYPE X Y WIDTH HEIGHT)
     (:SELF-FLAVOR ESSENTIAL-SCROLL-MOUSE-MIXIN))
  ;; Check bounds to make sure that (X,Y) is within this window.
  (WHEN (AND (>= Y (SHEET-INSIDE-TOP))
    ...
    (block HAVE-ITEM
      (PROG ((LINE-OF-INTEREST (SHEET-LINE-NO NIL Y))
	     (MOUSE-INFO) (LINE-ITEM) (LINE) (ENTRY)
	     (START-X (SHEET-INSIDE-LEFT)) (REL-Y) (FIRST-LINE))
	    (SETQ LINE-ITEM (AREF SCREEN-IMAGE LINE-OF-INTEREST 0))
	    (OR LINE-ITEM
		(RETURN-FROM HAVE-ITEM NIL))
	    (SETQ FIRST-LINE (SETQ LINE (- LINE-OF-INTEREST (AREF SCREEN-IMAGE LINE-OF-INTEREST 1))))
	    ...
	    (DOLIST (I (SCROLL-ITEM-MOUSE-ITEMS LINE-ITEM))
	      (AND (>= REL-Y (SCROLL-MOUSE-ITEM-REL-TOP I))
		   ...
		   (RETURN-FROM HAVE-ITEM (SCROLL-MOUSE-ITEM-ITEM I)
				(SCROLL-MOUSE-ITEM-TYPE I) START-X
		...
		((> START-X X)
		 (RETURN NIL))
		(T
		 (RETURN-FROM HAVE-ITEM (SETQ MOUSE-INFO (SCROLL-ENTRY-MOUSE-INFO ENTRY))
			      (FIRST MOUSE-INFO) START-X
	    ...
	    (COND
	      ((SETQ MOUSE-INFO (SCROLL-ITEM-LINE-SENSITIVITY LINE-ITEM))
	       (RETURN-FROM HAVE-ITEM MOUSE-INFO (FIRST MOUSE-INFO) (SHEET-INSIDE-LEFT)
			    (+ (* FIRST-LINE LINE-HEIGHT) (SHEET-INSIDE-TOP)) (SHEET-INSIDE-WIDTH)
			    (DO ((I (1+ FIRST-LINE) (1+ I)))
				((OR (>= I SCREEN-LINES)
				     (NEQ LINE-ITEM (AREF SCREEN-IMAGE I 0)))
...
(DEFMETHOD (ESSENTIAL-SCROLL-MOUSE-MIXIN :MOUSE-CLICK)
	   (BUTTON X Y &AUX ITEM TYPE OP)
  (COND ((ZEROP (CHAR-MOUSE-CLICKS BUTTON))
         (MULTIPLE-VALUE-SETQ (ITEM TYPE)
           (SEND SELF :MOUSE-SENSITIVE-ITEM X Y))
         (COND
           ((NULL ITEM) NIL)
           ((OR (NULL TYPE)
                (SETQ OP (FIRST (CDR (ASSOC TYPE TYPE-ALIST :TEST #'EQ)))))
...
(DEFMETHOD (ESSENTIAL-SCROLL-MOUSE-MIXIN :WHO-LINE-DOCUMENTATION-STRING) ()
  (AND (CONSP CURRENT-ITEM) (CONSP (CDR CURRENT-ITEM))
